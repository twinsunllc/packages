...
+ (nullable FLTIOSMediaSelectionTypeData *)nullableFromMap:(NSDictionary *)dict;
- (NSDictionary *)toMap;
@end
...
+ (nullable FLTIOSMediaSelectionTypeData *)nullableFromMap:(NSDictionary *)dict { return (dict) ? [FLTIOSMediaSelectionTypeData fromMap:dict] : nil; }
- (NSDictionary *)toMap {
  return @{
    @"value" : @(self.value),
  };
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:FLTImagePickerApiGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(pickMediaWithMaxSize:quality:allowMultiple:allowedTypes:completion:)], @"FLTImagePickerApi api (%@) doesn't respond to @selector(pickMediaWithMaxSize:quality:allowMultiple:allowedTypes:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTMaxSize *arg_maxSize = GetNullableObjectAtIndex(args, 0);
        NSNumber *arg_imageQuality = GetNullableObjectAtIndex(args, 1);
        NSNumber *arg_allowMultiple = GetNullableObjectAtIndex(args, 2);
        NSArray<FLTIOSMediaSelectionTypeData *> *arg_allowedTypes = GetNullableObjectAtIndex(args, 3);
        [api pickMediaWithMaxSize:arg_maxSize quality:arg_imageQuality allowMultiple:arg_allowMultiple allowedTypes:arg_allowedTypes completion:^(NSArray<NSString *> *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    }
